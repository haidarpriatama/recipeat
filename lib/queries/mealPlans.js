...
function getMealPlanSelect(dayName, userId) {
  return {
    id: true,
    recipes: {
...
function mapMeals(mealPlan, dayName) {
  ...
  return {
    meals: mealPlan.recipes.map((entry) => ({
      ...
      rating: entry.recipe.ratings[0]?.score ? entry.recipe.ratings[0].score.toFixed(1) : "0.0",
    })),
    mealPlanId: mealPlan.id,
    dayName,
  };
...
export async function getMealsForDate({ email }, dateStr) {
  ...
  let mealPlan = await prisma.mealPlan.findFirst({
    ...
    select: getMealPlanSelect(dayName, dbUser.id),
  });
