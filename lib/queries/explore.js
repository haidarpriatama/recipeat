 ...
 */
function shapeRecipe(recipe, favoriteSet = new Set(), ratingMap = new Map()) {
  return {
    id: recipe.id,
    title: recipe.title,
    ...
    rating: (ratingMap.get(recipe.id) || 0).toFixed(1),
...
export async function fetchAuthenticatedExploreRecipes(filters, page = 1, userId) {
  ...
  return {
    recipes: recipes.map((r) => shapeRecipe(r, favoriteSet, ratingMap)),
    total,
  };
}
